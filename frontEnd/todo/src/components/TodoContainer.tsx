...
const TodoContainer = ({dataIsAdded, setDataIsAdded, setEditData, setInputValue, setDataId, setCompleted}: TodoContainerProps) => {
  const [getTodoData, setGetTodoData] = useState<GetTodoData[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/todoApp/getTodoData', {
  ...
  const deleteTodoRow = async (id: string) => {
    try {
      await axios.delete(`http://localhost:5000/todoApp/deleteTodoData/${id}`, {
        headers: { 'Cache-Control': 'no-cache' }, // Disable caching for this request
      });
  ...
  const completeRow = async (id: string, completed: boolean) => {
    try {
      await axios.put('http://localhost:5000/todoApp/completeRow', {_id: id, completed: !completed}, {
    ...
    <div className='todoItemContainer'>
      ...
      </div>
      {
        getTodoData?.map((item, index) => {
          return (
            <TodoItem item={item} index={index} key={item._id} deleteTodoRow={deleteTodoRow} editFunction={editFunction} completeRow={completeRow}/>
          )
        })
      }
    </div>
