...
const TodoInput = ({
  ...
  const changeData = async (e: React.FormEvent) => {
    ...
    try {
      if (!editData) {
        await axios.post(
          "https://mern-todo-app-jwow.onrender.com/todoApp",
          { title: inputValue, completed },
          {
        ...
        await axios.put(
          "https://mern-todo-app-jwow.onrender.com/todoApp/changeTodoData",
          { id: dataId, editedTitle: inputValue, completed },
          {
            headers: { "Cache-Control": "no-cache" },
          }
        );
